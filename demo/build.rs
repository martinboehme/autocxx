...
fn main() {
    ...
    let deppath = std::path::PathBuf::from("or-tools/dependencies/install/include").canonicalize().unwrap();
    let genpath = std::path::PathBuf::from("or-tools/ortools/gen").canonicalize().unwrap();
